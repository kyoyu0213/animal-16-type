    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(245,158,11,0.14),transparent_20%),linear-gradient(180deg,#050816,#111827)] px-4 py-10 text-slate-100">
      <div className="mx-auto max-w-6xl rounded-[2rem] border border-amber-400/10 bg-slate-950/90 p-8 shadow-soft ring-1 ring-amber-400/10 backdrop-blur-sm">
        ...
        {result ? (
          ...
          <div className="rounded-[1.75rem] border border-amber-300/20 bg-slate-950/90 p-8 text-center text-slate-300">
            診断結果が見つかりません。もう一度診断を受けてください。
          </div>
        )}
        ...
        {copied && (
          <div className="mt-4 text-center text-sm text-emerald-500">リンクをコピーしました！</div>
        )}
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <Link href="/questions" className="inline-flex w-full items-center justify-center rounded-full border border-amber-300 bg-slate-900 px-6 py-3 text-sm font-semibold text-amber-100 transition hover:bg-slate-800">
            もう一度診断する
          </Link>
          <Link href="/" className="inline-flex w-full items-center justify-center rounded-full border border-amber-300 bg-slate-900 px-6 py-3 text-sm font-semibold text-amber-100 transition hover:bg-slate-800">
            トップに戻る
          </Link>
        </div>
      </div>
